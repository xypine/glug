...
impl Sender {
    fn name(&self) -> String {
        match self {
            Sender::From { from, from_id } => from.clone(),
            Sender::Actor { actor, actor_id } => actor.clone(),
        }
    }

    fn id(&self) -> String {
        match self {
            Sender::From { from, from_id } => from_id.clone(),
            Sender::Actor { actor, actor_id } => actor_id.clone(),
        }
    }
}
